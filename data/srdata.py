    ...
    def __init__(self, args, train=True, benchmark=False):
        ...
        if args.ext == 'img' or benchmark:    # benchmark --> 测试集 或者 img
            ...
            try:
                ...
                bin_path = os.path.join(self.apath, 'bin')    # self.apath = dir_data + '/DIV2K'
                if not os.path.isdir(bin_path):
                    os.mkdir(bin_path)

                list_hr, list_lr = self._scan()
                print('Reading {} HR images'.format(args.data_train))
                hr = [misc.imread(f) for f in tqdm(list_hr)]
                np.save(self._name_hrbin(), hr)               # hr 是个 list 存储了所有的图片信息
                del hr
                for si, s in enumerate(self.scale):
                    print('Reading {0} LR_X{1} images'.format(args.data_train, self.scale[si]))
                    lr_scale = [misc.imread(f) for f in tqdm(list_lr[si])]
